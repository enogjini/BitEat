    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="bg-white rounded-3xl shadow-xl p-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-black"><Table className="inline text-orange-600" /> Rezervime</h2>
        ...
        {showForm && (
          <div className="bg-orange-50 p-6 rounded-2xl mb-6 border-2 border-orange-200">
            <h3 className="text-xl font-black mb-4 text-orange-800">Rezervim i Ri</h3>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={formData.emri_klientit} onChange={(e) => setFormData({...formData, emri_klientit: e.target.value})}
                  className="w-full p-3 rounded-xl border-2 border-orange-200 outline-none" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="date" value={formData.data_rezervimit} onChange={(e) => setFormData({...formData, data_rezervimit: e.target.value})}
                  min={new Date().toISOString().split('T')[0]} className="w-full p-3 rounded-xl border-2 border-orange-200 outline-none" />
              </div>
              <div>
              ...
              <div className="col-span-2">
                ...
                <textarea value={formData.shenim} onChange={(e) => setFormData({...formData, shenim: e.target.value})}
                  rows="2" className="w-full p-3 rounded-xl border-2 border-orange-200 outline-none resize-none" />
              </div>
            </div>
            <div className="flex gap-3 mt-4">
              ...
              <button onClick={() => setShowForm(false)} className="px-6 bg-slate-300 p-4 rounded-xl font-black">ANULO</button>
            </div>
          </div>
        )}
        ...
        <div className="space-y-3">
          {rezervime.map(r => (
            <div key={r.rezervim_id} className="flex justify-between items-center p-4 bg-slate-50 rounded-xl">
              ...
              <div className="flex items-center gap-3">
                <span className={`px-4 py-2 rounded-full font-bold text-sm ${
                  r.statusi === 'E konfirmuar' ? 'bg-green-100 text-green-800' : 
                  r.statusi === 'E anuluar' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'
                }`}>
                  {r.statusi}
                </span>
